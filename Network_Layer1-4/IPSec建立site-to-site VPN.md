...
- 採坑︰winbox ip/nat設定標籤中Extra展開與未展開設定是不一樣的要注意，設定時因為認為展開不做設定就是不生效導致nat失敗（展開代表已經生效）
- 採坑：在windows設定L2TP/IPsec始終無法連線，將設定檔刪掉重新設定便可以連線ROS VPN，RouterOS IPSec設定更改後，windows vpn最好刪掉重新設定
...
- IP-Pools的要做 policy routing，in-interface不用指定介面

## site-to-site設定步驟

1. 本機外部路由要在 main
2. ipsec/profiles 建立配置文件（ike的步驟一，Phase1）  
