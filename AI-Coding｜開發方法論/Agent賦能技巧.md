# Agents賦能

自訂 Agent，賦予AI能力

## 通則

- instructions由少到多，避免模型上下文汙染

## 注意事項

- 避免上下文汙染

## 賦能類型

1. SDD Agent
2. Skills
3. MCP
4. 模型
5. Hooks
6. 工具 extensions/Plugins
7. Prompt
8. Agents

## Marketplace

- [The Agent Skills Directory](https://skills.sh/)
- [MCP Marketplace - Cline](https://cline.bot/mcp-marketplace)
- [Agent Skills Marketplace - Claude, Codex & ChatGPT Skills | SkillsMP](https://skillsmp.com/)
- [Discover Top MCP Servers | MCP Market](https://mcpmarket.com/)
- [MCPMark - Stress-Testing Comprehensive MCP Benchmark](https://mcpmark.ai/)
- [travisvn/awesome-claude-skills: A curated list of awesome Claude Skills, resources, and tools for customizing Claude AI workflows — particularly Claude Code](https://github.com/travisvn/awesome-claude-skills?tab=readme-ov-file)
- [Awesome Claude Skills - Visual Directory - Awesome Claude](https://awesomeclaude.ai/awesome-claude-skills)
- [ComposioHQ/awesome-claude-skills: A curated list of awesome Claude Skills, resources, and tools for customizing Claude AI workflows](https://github.com/ComposioHQ/awesome-claude-skills)
- [SkillHub - Claude Skills & Agent Skills Marketplace | Claude Code Skills](https://www.skillhub.club/)
- [claude-skills-hub · GitHub Topics](https://github.com/topics/claude-skills-hub)
- [Claude Code Skills Library - Browse All Anthropic Claude Skills](https://claudeskills.info/skills/)
- [anthropics/skills: Public repository for Agent Skills](https://github.com/anthropics/skills?tab=readme-ov-file)
- [vercel-labs/agent-skills: Vercel's official collection of agent skills](https://github.com/vercel-labs/agent-skills)
- [Context7 - Up-to-date documentation for LLMs and AI code editors](https://context7.com/)
- [sickn33/antigravity-awesome-skills: The Ultimate Collection of 800+ Agentic Skills for Claude Code/Antigravity/Cursor. Battle-tested, high-performance skills for AI agents including official skills from Anthropic and Vercel.](https://github.com/sickn33/antigravity-awesome-skills)
- [vercel/next-devtools-mcp: Next.js Development for Coding Agent](https://github.com/vercel/next-devtools-mcp#readme)
- [nextlevelbuilder/ui-ux-pro-max-skill: An AI SKILL that provide design intelligence for building professional UI/UX multiple platforms](https://github.com/nextlevelbuilder/ui-ux-pro-max-skill)

## MCP 蒐集

- [oli799/lucide-svg-mcp](https://github.com/oli799/lucide-svg-mcp)
- [jeff-nasseri/mikrotik-mcp: MCP server for Mikrotik](https://github.com/jeff-nasseri/mikrotik-mcp?tab=readme-ov-file)
- [sevaepsteyn/routeros_mcp: MCP server for MikroTik RouterOS devices](https://github.com/sevaepsteyn/routeros_mcp)
- [halilural/electron-mcp-server](https://github.com/halilural/electron-mcp-server)

## Skills 蒐集

### 系統與網路

- [mikrotik-routeros-rsc by alexandre-machado/ai-stuffs](https://skills.sh/alexandre-machado/ai-stuffs/mikrotik-routeros-rsc)
- [powershell-7-expert by 404kidwiz/claude-supercode-skills](https://skills.sh/404kidwiz/claude-supercode-skills/powershell-7-expert)

### 程式開發

- [electron by teachingai/full-stack-skills](https://skills.sh/teachingai/full-stack-skills/electron)
- [Bun from bun.sh](https://skills.sh/site/bun.sh/bun)
- [bun-development by sickn33/antigravity-awesome-skills](https://skills.sh/sickn33/antigravity-awesome-skills/bun-development)
- [suggest-lucide-icons by nweii/agent-stuff](https://skills.sh/nweii/agent-stuff/suggest-lucide-icons)
- [gh-cli by github/awesome-copilot](https://skills.sh/github/awesome-copilot/gh-cli)

## 學習資源

- [How to Build a Custom MCP Server with TypeScript – A Handbook for Developers](https://www.freecodecamp.org/news/how-to-build-a-custom-mcp-server-with-typescript-a-handbook-for-developers)
- [Agent Skills傻瓜式教程！26年最火AI技术就这？ - YouTube](https://www.youtube.com/watch?v=8NGznVwNHGY)
- [neondatabase/add-mcp: The open MCP config tool - npx add-mcp](https://github.com/neondatabase/add-mcp)
- [Index - Agent Development Kit (ADK)](https://google.github.io/adk-docs/)
 - [用typescript建立簡單的MCP應用. 先建立node專案打開ternimal下指令 | by 海螺 | Medium](https://medium.com/@yojjoyy1/%E7%94%A8typescript%E5%BB%BA%E7%AB%8B%E7%B0%A1%E5%96%AE%E7%9A%84mcp%E6%87%89%E7%94%A8-a4c7d6e39f18)
 - [代理技能 (Agent Skills) 是什麼? 解決了什麼問題?｜ExplainThis](https://www.explainthis.io/zh-hant/ai/agent-skills)
 - [AGENTS.md](https://agents.md/)
 - [Overview - Agent Skills](https://agentskills.io/home)
 - [Model Context Protocol (MCP)](https://modelcontextprotocol.io/docs/getting-started/intro)｜[Model Context Protocol](https://github.com/modelcontextprotocol)
 - [CursorTouch/Windows-MCP: MCP Server for Computer Use in Windows](https://github.com/CursorTouch/Windows-MCP)
 - [Claude Code Skills：讓 AI 變身專業工匠 | 高見龍](https://kaochenlong.com/claude-code-skills)
 - [深度解析 Electron MCP Server：賦能 AI 代理自動化桌面應用的終極指南](https://skywork.ai/skypage/zh-hant/%E6%B7%B1%E5%BA%A6%E8%A7%A3%E6%9E%90-Electron-MCP-Server%EF%BC%9A%E8%B3%A6%E8%83%BD-AI-%E4%BB%A3%E7%90%86%E8%87%AA%E5%8B%95%E5%8C%96%E6%A1%8C%E9%9D%A2%E6%87%89%E7%94%A8%E7%9A%84%E7%B5%82%E6%A5%B5%E6%8C%87%E5%8D%97/1971038599896952832)
 - [microsoft/mcp-for-beginners: This open-source curriculum introduces the fundamentals of Model Context Protocol (MCP) through real-world, cross-language examples in .NET, Java, TypeScript, JavaScript, Rust and Python. Designed for developers, it focuses on practical techniques for building modular, scalable, and secure AI workflows from session setup to service orchestration.](https://github.com/microsoft/mcp-for-beginners?tab=readme-ov-file)
 - [[2025 更新] 開發者必備！ 15 個 MCP 伺服器協助 Claude Code 和 Cursor 效能提升](https://codelove.tw/@tony/post/an202a)
 - [Agentic AI Roadmap 2025](https://www.facebook.com/100064701364734/posts/1219662160200485)

 - [What is the Model Context Protocol (MCP)? - Model Context Protocol](https://modelcontextprotocol.io/docs/getting-started/intro)
 - [GitHub - modelcontextprotocol/servers: Model Context Protocol Servers](https://github.com/modelcontextprotocol/servers)
 - [appcypher/awesome-mcp-servers: Awesome MCP Servers - A curated list of Model Context Protocol servers](https://github.com/appcypher/awesome-mcp-servers)
 - [MCP Servers－mcp.so](https://mcp.so/)
 - [mcp是什麼？mcp server是什麼？mcp中文、意思、實例一次看懂|數位時代 BusinessNext](https://www.bnext.com.tw/article/82706/what-is-mcp)
 - [Cline - AI Coding, Open Source and Uncompromised](https://cline.bot/)
 - [UI 設計師必看：Cursor + Figma MCP 的 AI 神技!](https://tenten.co/learning/cursor-figma-mcp/)
 - [張文鈿 - MCP 生態發展趨勢](https://www.facebook.com/562573970/posts/10162738966963971)

## API vs MCP

MCP (模型上下文協定) 與 API (應用程式介面) 旨在解決不同層面的整合問題。==**API** 是為機器間通信設計的靜態規則，而 **MCP** 則是專為 AI 代理設計的動態標準，讓 LLM 能動態理解並使用工具==。MCP 通常作為 API 的包裝器 (Wrapper)，使其更友善地接入 AI，而非取代傳統 API。 

**關鍵差異與詳細分析**

- **目標使用者**：API 主要供開發人員編寫代碼使用；MCP 則是設計給 LLM 直接呼叫工具。
- **靈活性與動態性**：API 的功能需要人工事先定義；MCP 允許 AI 代理程式動態探索、適應並調用工具。
- **抽象層級**：API 提供底層數據的存取點；MCP 提供上層的語境 (Context)，幫助 AI 理解如何使用這些數據。
- **合作關係**：MCP 經常利用現有的 REST API 進行包裝，讓複雜的 API 操作變成 AI 可理解的工具。 

**MCP vs API 比較表**

|特性|傳統 API (如 REST)|MCP (Model Context Protocol)|
|---|---|---|
|**主要使用者**|開發人員 (Human)|AI 模型/代理 (Agent)|
|**設計哲學**|靜態、結構化|動態、上下文感知|
|**工作流程**|需人工定義每個接口|AI 自主適應工具|
|**互動模式**|機器對機器 (M2M)|AI 對機器 (AI2M)|
|**適用場景**|底層數據操作、系統集成|LLM 工具使用、多源數據整合|

**為什麼需要 MCP？**  
雖然 API 很成熟，但 LLM 往往難以直接理解复杂的技術文檔。MCP 透過標準化介面，解決了「AI 如何輕鬆安全地連接外部數據和工具」的問題。

## 李弘毅 教授

- [【生成式AI時代下的機器學習(2025)】第一講：一堂課搞懂生成式人工智慧的技術突破與未來發展](https://www.youtube.com/watch?v=QLiKmca4kzI)
- [【生成式AI時代下的機器學習(2025)】第二講：一堂課搞懂 AI Agent 的原理 (AI如何透過經驗調整行為、使用工具和做計劃)](https://www.youtube.com/watch?v=M2Yg1kwPpts)
- [MCP是什么？为啥是下一代AI标准？MCP原理+开发实战！在Cursor、Claude、Cline中使用MCP，让AI真正自动化！](https://www.youtube.com/watch?v=jGVsLeDxtQY)

## 程序員老王

- [10分钟弄懂 什么是大模型Skill - YouTube](https://www.youtube.com/watch?v=lnneAfJqd9M)
- [10分钟讲清楚 Prompt, Agent, MCP 是什么](https://www.youtube.com/watch?v=1UQ8uz4kuIc)
- [AI 提示词工程 上下文工程 15分钟弄懂！](https://www.youtube.com/watch?v=-8Ygq9AVWZ8)

## 馬克技術工作坊

- [MCP终极指南 - 从原理到实战，带你深入掌握MCP（基础篇）](https://www.youtube.com/watch?v=yjBUnbRgiNs)
- [MCP终极指南 带你深入掌握MCP（进阶篇）](https://www.youtube.com/watch?v=zrs_HWkZS5w)
- [MCP终极指南 - 番外篇：抓包分析 Cline 与模型的交互协议（内含 Agent 的实现原理）](https://www.youtube.com/watch?v=YyVkXrXxvX8)
- [RAG 工作机制详解——一个高质量知识库背后的技术全流程](https://www.youtube.com/watch?v=WWdlme1EAGI)
- [（影片：28min）Agent 的概念、原理與構建模式 —— 從零打造一個簡化版的 Claude Code](https://www.youtube.com/watch?v=GE0pFiFJTKo)
- [Context Engineering：概念與技術實現深度解析](https://www.youtube.com/watch?v=25DEMZ7wsSM)

## n8n

 - [PAPAYA 電腦教室 n8n EP1](https://www.youtube.com/watch?v=r9mi3ZJIWbg)
 - [PAPAYA 電腦教室 n8n EP2](https://www.youtube.com/watch?v=sRU6Y7DXkLI)
 - [PAPAYA 電腦教室 n8n EP3](https://www.youtube.com/watch?v=D-VEcKZ3NV0)
- [有 LLM 幹嘛還要 n8n？](https://www.facebook.com/leo21c/posts/pfbid0ftuQGp2F88adeU1n91NE5eZXBN5tPNYNhPvWnwqhLx85C9tADLB2jwKSjuXejs2Gl)
- [我的確覺得那些自動化平台像 Zaiper、n8n 可能在莫名的狀況下就開始被略過了](https://www.facebook.com/649368317/posts/10161417606668318)
 - [n8n 還是 Make.com？一個過來人的真心話](https://www.facebook.com/668215801/posts/10163611591815802)
 - [n8n+AI Agent:自動化應用指南 - YouTube](https://www.youtube.com/playlist?list=PL317zS4Ng0zbfLj-jDoEJgpA-hxcORldn)
